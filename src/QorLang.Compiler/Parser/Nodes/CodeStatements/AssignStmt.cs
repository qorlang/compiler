...
) : CodeStmt(location)
{
	...
	{
		if (obj is not AssignStmt other) return false;
	...
	public override string ToString() => JsonSerializer.Serialize(new { type = nameof(AssignStmt), name = Target, value = JsonDocument.Parse(Value.ToString()).RootElement });
}