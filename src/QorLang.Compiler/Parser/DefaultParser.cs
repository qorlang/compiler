...
public class DefaultParser(IEnumerable<Token> tokens)
{
	...
	(ASTNode typeRefNode, Token lastUnprocessedToken) ParseTypeReference()
	{
		...
		else if (token.Type == TokenType.Keyword)
		{
			...
			if (typeName == "::void")
			{
				return (TypeReferenceNode.Void(), default);
			}
	...
	/// <returns></returns>
	ASTNode ParseFunctionDefinition(string functionName)
	{
		...
		if (!ExpectNextToken(TokenType.Colon, out var parsedToken))
		{
			returnType = TypeReferenceNode.Void();
			RemoveLastError(); // remove the error generated by the missing colon

			if (parsedToken.Type != TokenType.LeftBrace)
			{
				_errors.Add(GenerateError($"Error QR202: Expected ':' or '{{' after parameter list, but got {TokenRepr.ToString(parsedToken)} instead.", parsedToken));
